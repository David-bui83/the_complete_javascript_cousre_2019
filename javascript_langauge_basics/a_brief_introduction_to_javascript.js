/**
    JavaScript is a lightweight, crossplatform, object-oriented computer programming language
    
    JavaScript is one of the three core technologies of web development
    
    Today, JavaScript can be used in different places:
        Client-side: JavaScript was traditionally in the browser
        Server-side: Thanks to node.js, we can use JavaScript on the server as well
        
    JavaScript is what made modern web development possible:
        Dynamic effects and interactivity
        Modern web applications that we can interact with
    
    Frameworks/libraries like React and Angular are 100% based on JavaScript: you
    need to master JavaScript in order to use them!
    
    HTML is the nouns - <p></p> means 'paragraph'
    CSS is the adjectives - p {color: red; } means 'the paragraph text is red'
    JS is the verbs - p.hide(); means 'hide the paragraph'
    
*/